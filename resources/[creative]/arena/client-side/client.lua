...
local Zones = {
	["1"] = {
		["Ped"] = {
			...
			["Coords"] = vec3(214.53,-896.13,30.68),
			["Heading"] = 240.49
		},
		["Respawn"] = {
			...
			{ -311.51,247.02,283.67 },
			{ -311.44,223.01,283.67 },
			{ -311.34,201.0,283.67 },
			{ -311.19,173.94,283.67 },
			{ -293.02,162.8,283.67 },
			{ -272.08,163.14,283.67 },
			{ -244.81,164.34,283.67 },
			{ -241.39,189.12,283.67 },
			{ -239.8,216.89,283.67 },
			{ -240.48,242.83,283.67 },
			{ -275.26,248.08,283.67 },
			{ -275.65,211.11,283.67 },
			{ -275.36,197.52,283.67 },
			{ -288.0,193.46,283.67 },
			{ -255.83,194.15,283.67 },
			{ -292.36,216.28,283.67 }
		},
		["Exit"] = { 219.25,-899.11,30.68 },
		["Route"] = 900001,
		["PolyZone"] = PolyZone:Create({
			...
			-- vector2(-1042.00,-482.13), -314.61,250.15
			-- vector2(-1043.93,-483.11),
			-- vector2(-1035.33,-482.96),
			-- vector2(-1025.11,-479.39)
			vector2(-314.17,249.35),
			vector2(-314.55,160.9),
			vector2(-236.39,160.95),
			vector2(-236.57,249.13)
		},{ name = "Arena-1" })
	}
}
-----------------------------------------------------------------------------------------------------------------------------------------
...
RegisterNetEvent("arena:rPlayers")
AddEventHandler("arena:rPlayers",function(Route,Amount)
	if LocalPlayer["state"]["Route"] == Route then
		Players = 1
		SendNUIMessage({ Players = Players - 1, Streek = KillStreek })
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- ARENA:RESETSTREEK
...
-- 		AddTextComponentString("Arena PVP")
