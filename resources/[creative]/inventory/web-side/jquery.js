	...
	$(".populated").tooltip({
		create: function(event,ui){
			...
			if (name == "Passaporte" || name == "Distintivo"){
				...
				var idGems = $(this).attr("data-idGems");
				var idBlood = $(this).attr("data-idBlood");
				var idPremium = $(this).attr("data-idPremium");
				var idRolepass = $(this).attr("data-idRolepass");
				var idNacionalidade = $(this).attr("data-idNacionalidade");

				contents = `<item>${name}</item>${description !== "undefined" ? "<br><description>"+description+"</description>":""}<br><legenda>Nome: <r>${idName}</r> <s>|</s> Porte: <r>${idPort == 0 ? "Não":"Sim"}</r><br>Tipo Sangüineo: <r>${idBlood}</r> <s>|</s> Gemas: <r>${formatarNumero(idGems)}</r><br>Nativo: <r>${idNacionalidade}</r> <s>|</s> Passe: <r>${idRolepass}</r>${idPremium == "Nenhum" ? "":"<br>Premium: <r>"+ idPremium +"</r>"}</legenda>`;
			}
