		...
		if IsPedInAnyVehicle(ped) then
			...
			if bikesModel[vehModel] and GetGameTimer() >= bikesTimer and speed >= 10 then
				...
				if bikesPoints >= bikeMaxPoints then
					...
					TriggerEvent("Notify","verde","Você aumentou sua capacidade máxima de Carga, continue se Exercitando.",10000)
					bikesPoints = 0
				end
			end
		end
...
-----------------------------------------------------------------------------------------------------------------------------------------
Citizen.CreateThread(function()
    imageUrl = "https://cdn.discordapp.com/attachments/461546018971058176/999540640629129327/dsadsdf.png" 
   ...
   -- AddReplaceTexture('vehshare', 'plate05', 'duiTxd', 'duiTex') 
    ...
    local object = CreateDui('https://cdn.discordapp.com/attachments/461546018971058176/999539854167789678/fafafa.png', 540, 300) 
    local handle = GetDuiHandle(object)
