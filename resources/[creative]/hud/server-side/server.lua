			...
			if callTimers[user_id] <= os.time() and source > 0 then
				...
				for k,v in pairs(policeResult) do
					async(function()
						...
						TriggerClientEvent("NotifyPush",v,{ code = 20, title = "Digitais Encontradas", x = coords["x"], y = coords["y"], z = coords["z"], criminal = "Alerta de procurado", time = "Recebido às "..os.date("%H:%M"), blipColor = 16 })
					end)
				end
			end
