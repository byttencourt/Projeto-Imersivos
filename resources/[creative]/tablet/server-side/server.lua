		...
		if myVehicle[1] then
			...
			if identity then
				...
				if parseInt(maxVehs[1]["qtd"]) >= parseInt(amountVehs) then
					TriggerClientEvent("Notify",source,"amarelo","Atingiu o máximo de veículos.",5000)
					return
				end
				...
				if vRP.paymentGems(user_id,vehPrice) then
					...
					if vehicle[1] then
						if vehicle[1]["rental"] <= os.time() then
							...
							TriggerEvent("discordLogs","buycar","**Passaporte:** "..user_id.."\n**Adicionou 30 dias aluguel ao veículo:** "..vehicleName(vehName).."\n**Valor:** "..parseFormat(vehPrice).." gemas\n**Horário:** "..os.date("%H:%M:%S"),3092790)
						...
						TriggerEvent("discordLogs","buycar","**Passaporte:** "..user_id.."\n**Alugou veículo:** "..vehicleName(vehName).."\n**Valor:** "..parseFormat(vehPrice).." gemas\n**Horário:** "..os.date("%H:%M:%S"),3092790)
					end
				else
				...
				if vehicleType(vehName) == "work" then
					...
					if vRP.request(source,"Comprar <b>"..vehicleName(vehName).."</b> por <b>$"..parseFormat(vehPrice).."</b> dólares?") then
						...
						if vRP.paymentFull(user_id,vehPrice) then
							...
							TriggerEvent("discordLogs","buycar","**Passaporte:** "..user_id.."\n**Comprou:** "..vehicleName(vehName).."\n**Valor:** "..parseFormat(vehPrice).." dólares.\n**Horário:** "..os.date("%H:%M:%S"),3092790)
