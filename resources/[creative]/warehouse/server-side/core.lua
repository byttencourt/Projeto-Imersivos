	...
	if user_id then
		...
		if consult[1] == nil then
			...
			if vRP.request(source,"Deseja comprar este <b>Armazém</b> por <b>$"..parseFormat(buyPrice).."</b>?") then
				...
				local Password = sanitizeString(newWarehouses[1],"0123456789",true)
			...
			if consult[1] then
				if consult[1]["tax"] > os.time() then
					...
					if vRP.request(source,"Pagar o aluguel do armazém por <b>$20.000</b>?","Sim, por favor","Não, decido depois") then
						if vRP.paymentFull(user_id,20000) then
							vRP.execute("warehouses/Tax",{ name = Number })
							return true
						else
							TriggerClientEvent("Notify",source,"vermelho","<b>Dólares</b> insuficientes.",5000)
						end
	...
	if user_id then
		...
		if consultWarehouses[1] then
			...
			if consultOwner[1] then
				...
				local Password = sanitizeString(newPassword[1],"0123456789",true)
				if string.len(Password) >= 4 and string.len(Password) <= 20 then
