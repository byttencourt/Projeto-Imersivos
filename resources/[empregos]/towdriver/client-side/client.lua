...
-----------------------------------------------------------------------------------------------------------------------------------------
local vehModels = {
	"baller","jackal","youga","mesa","nemesis","primo","bison","seminole","hotring","landstalker","panto",
	"premier","rhapsody","pcj","jester","superd","sentinel","sentinel2","blazer2","asea",
	"regina","huntley","tornado","patriot3","bjxl","patriot","ingot","serrano","vstr","bobcatxl",
	"journey","bfinjection","sanchez2","surfer2","caddy2","rebel2","bagger","dilettante","blista","hexer",
	"buffalo","emperor2","fugitive","rocoto","dukes","thrust","double","camper","massacro","feltzer2",
...
AddEventHandler("towdriver:Toggle",function(id)
	if inService then
		inService = false
		TriggerEvent("Notify","azul","Saiu de serviço.",5000)
	else
		inService = true
		serviceLocate = id
		...
		TriggerEvent("NotifyPush",{ code = 20, title = "Registro de Veículo", x = vehRescue[serviceLocate][spawnCoords][1], y = vehRescue[serviceLocate][spawnCoords][2], z = vehRescue[serviceLocate][spawnCoords][3], vehicle = "Veículo: "..vehName, name = "Aguardando reboque", blipColor = 2 })
		TriggerEvent("Notify","azul","Entrou em serviço.",5000)
	end
...
AddEventHandler("towdriver:invokeTow",function()
	...
	if IsVehicleModel(vehicle,GetHashKey("flatbed")) or IsVehicleModel(vehicle,GetHashKey("scaniarepair")) or IsVehicleModel(vehicle,GetHashKey("scaniarepair2")) and not IsPedInAnyVehicle(ped) then
