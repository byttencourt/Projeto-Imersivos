...
AddEventHandler("cemitery:initBody",function()
	...
	if spawnPed == nil then
		...
		if HasModelLoaded(mHash) then
			...
			TriggerEvent("Notify","amarelo","Parece que estão efetuando uma limpeza em um dos túmulos, guarde essa informação e veja se você encontra alguns objetos de valor.",20000)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
AddEventHandler("cemitery:finishBody",function()
	...
	if checkItem >= 5 then
		exports["target"]:RemCircleZone("Cemitery:"..Selected)
		TriggerEvent("Notify","amarelo","Este é o ultimo pertence desse presunto.",10000)
		Selected = nil
	end
end)