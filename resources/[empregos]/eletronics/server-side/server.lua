...
function cRP.checkSystems()
	...
	if user_id then
		...
		if parseInt(#policeResult) <= 1 or os.time() < atmTimers then
			...
			if vRP.tryGetInventoryItem(user_id,"floppy",1) then
				for k,v in pairs(policeResult) do
					async(function()
						vRPC.playSound(v,"ATM_WINDOW","HUD_FRONTEND_DEFAULT_SOUNDSET")
						TriggerClientEvent("NotifyPush",v,{ code = 31, title = "ATM - Caixa Eletrônico", x = coords["x"], y = coords["y"], z = coords["z"], criminal = "Alarme de segurança", time = "Recebido às "..os.date("%H:%M"), blipColor = 16 })
					end)
				end
			end
			return true
...
function cRP.paymentSystems(locate)
	...
	if user_id then
		...
		if os.time() < atmTimers then
			local randPrice = math.random(60,65)
