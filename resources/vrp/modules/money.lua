...
function vRP.addBank(user_id,amount,mode)
	...
	if parseInt(amount) > 0 then
		...
		local user_id = parseInt(user_id)
		vRP.execute("bank/addValue",{ user_id = user_id, value = amount, mode = "Private" })
...
function vRP.delBank(user_id,amount,mode)
	...
	if parseInt(amount) > 0 then
		local amount = parseInt(amount)
		local user_id = parseInt(user_id)
		vRP.execute("bank/remValue",{ user_id = user_id, value = amount, mode = "Private" })
...
function vRP.setBank(user_id,amount)
	...
	if parseInt(amount) > 0 then
		local amount = parseInt(amount)
		local user_id = parseInt(user_id)
		vRP.execute("bank/setValue",{ user_id = user_id, value = amount, mode = "Private" })
...
function vRP.getFines(user_id)
	local user_id = parseInt(user_id)
	local fines = vRP.query("characters/getFines",{ id = user_id })
	if fines[1] then
		return fines[1]["fines"]
	else
		local identity = vRP.userIdentity(user_id)
		if identity then
