...
vRP.prepare("characters/getFines","SELECT fines FROM summerz_characters WHERE id = @id")
...
vRP.prepare("characters/createCharacters","INSERT INTO summerz_characters(steam,name,name2,locate,sex,phone,serial,blood) VALUES(@steam,@name,@name2,@locate,@sex,@phone,@serial,@blood)")
-----------------------------------------------------------------------------------------------------------------------------------------
-- BANK
-----------------------------------------------------------------------------------------------------------------------------------------
...
vRP.prepare("bank/setValue","UPDATE summerz_bank SET value = @value WHERE user_id = @user_id AND mode = @mode AND owner = 1")
...
vRP.prepare("accounts/updatePremium2","UPDATE summerz_accounts SET premium = premium + 604800 WHERE steam = @steam")
vRP.prepare("accounts/setPremium","UPDATE summerz_accounts SET premium = @premium, priority = @priority WHERE steam = @steam")
vRP.prepare("accounts/dateLogin","UPDATE summerz_accounts SET login = @login, discord = @discord, last_ip = @last_ip  WHERE steam = @steam")
...
vRP.prepare("vehicles/rentalVehicles2","INSERT IGNORE INTO summerz_vehicles(user_id,vehicle,plate,work,rental,tax) VALUES(@user_id,@vehicle,@plate,@work,UNIX_TIMESTAMP() + 604800,UNIX_TIMESTAMP() + 604800)")
