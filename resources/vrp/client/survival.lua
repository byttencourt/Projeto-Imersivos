...
AddEventHandler("vRP:playerActive",function(user_id,name)
	...
	SetDiscordAppId(982114174001885204)
	SetDiscordRichPresenceAsset("logo-recuperado")
	SetRichPresence("#"..user_id.." "..name)--("#"..user_id.." "..name)
	SetDiscordRichPresenceAssetSmall("logo-recuperado")
	SetDiscordRichPresenceAssetText("discord.gg/imersivos")
	SetDiscordRichPresenceAssetSmallText("Imersivos Roleplay")
	SetDiscordRichPresenceAction(0, "Discord", "https://discord.gg/imersivos")
	SetDiscordRichPresenceAction(1, "Conectar !", "fivem://connect/cfx.re/join/qbrje4")
					...
					if LocalPlayer["state"]["Route"] < 900000 then
						if timeDeath <= 0 then
							if emergencyButton then
								SendNUIMessage({ deathtext = "Digite <color>/paramedico</color> para ser socorrido ou <color>/GG</color> para desistir imediatamente" })
							else
								if IsControlJustPressed(1,304) then
									TriggerServerEvent("help:Paramedic")
